package com.psl.common_productapi.controller;

import com.psl.common_productapi.model.Product;
import com.psl.common_productapi.service.ProductService;
...
public class ProductController {
    ...
    public Product saveProduct(@RequestPart("product") Product product, @RequestPart("image") MultipartFile file) {
        Product prod = null;
        try{
            prod = productService.saveProduct(Product.builder().title(product.getTitle()).price(product.getPrice()).image(ImageUtility.compressImage(file.getBytes())).build());
        }catch (IOException e){
            e.printStackTrace();
        }
    ...
    public ResponseEntity<Product> updateProduct(@PathVariable("id") Long id, @RequestPart("product") Product product, @RequestPart("image") MultipartFile file){
        ...
        try{
            if(product.getImage()==null) {
                prod = productService.updateProduct(Product.builder().title(product.getTitle()).price(product.getPrice()).image(ImageUtility.compressImage(file.getBytes())).build(), id);
            }else{
                prod = productService.updateProduct(Product.builder().title(product.getTitle()).price(product.getPrice()).image(ImageUtility.compressImage(product.getImage())).build(), id);
            }
        }catch (IOException e){
            e.printStackTrace();
        }
